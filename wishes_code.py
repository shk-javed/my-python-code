...
if os.path.exists(target_mp3):
    ...
    ydl_opts = {
        ...
        # Simplified approach - no cookies, just web client
        "extractor_args": {
            "youtube": {
                "player_client": ["web"],
            }
    ...
    try:
        ...
        print("2. Make sure Firefox is logged into YouTube")
        print("3. Try clearing Firefox cookies and logging in again")
        print("4. Alternative: Download manually and place as 'song.mp3' in this folder")
        sys.exit(1)
    ...
    if not os.path.exists(target_mp3):
        print("Error: song.mp3 not found after download")
        sys.exit(1)
...
def input_controls():
    global lyric_offset
    print("\nControls: p=pause, r=resume, s/q=stop, [/] nudge lyrics, o=show offset\n")
    while not stop_flag.is_set():
        ...
        if cmd == "p":
            ...
            print(f"⏱️  Current lyric offset: {lyric_offset:+.2f}s")
        elif cmd in ("s", "q"):
            stop_flag.set()
